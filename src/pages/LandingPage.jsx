...
export default function LandingPage() {
  ...
  useEffect(() => {
    fetch(`${baseUrl}config/landing.json`)
      .then(res => res.json())
      .then(data => setConfig(data))
      .catch(err => console.error('Error loading landing:', err));
  }, []);
        ...
        <div className="max-w-6xl mx-auto px-4 py-16 md:py-24">
          <div className="text-center">
            <h1 className="text-4xl md:text-6xl font-bold mb-6" 
              style={{ fontFamily: 'Playfair Display, serif', color: '#b56576' }}>
              {config.brandName}
            </h1>
      ...
      <section className="py-16 bg-pink-50">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="space-y-6">
            {config.howToOrder.map((item, index) => (
              <div key={index} className="flex items-center gap-4 bg-white p-6 rounded-2xl shadow">
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {config.pricing.map((pkg, index) => (
              <div key={index} className={`bg-white rounded-2xl shadow-xl p-8 relative ${pkg.popular ? 'ring-2 ring-pink-400 scale-105' : ''}`}>
                {pkg.popular && (
      ...
      <section className="py-16 bg-linear-to-r from-pink-400 to-rose-400">
        <div className="max-w-3xl mx-auto px-4 text-center text-white">
          ...
          <p className="text-lg mb-8 opacity-90">
            Pesan sekarang dan dapatkan undangan digital premium dalam 1-3 hari kerja
          </p>
          <a href={whatsappLink} target="_blank" rel="noreferrer"
        ...
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="flex justify-center gap-4 mb-4">
            <a href={`https://instagram.com/${config.contact.instagram}`} target="_blank" rel="noreferrer"
              className="hover:text-pink-400 transition">📸 Instagram</a>
            <a href={`https://wa.me/${config.contact.whatsapp}`} target="_blank" rel="noreferrer"
              className="hover:text-pink-400 transition">💬 WhatsApp</a>
            <a href={`mailto:${config.contact.email}`}
              className="hover:text-pink-400 transition">📧 Email</a>
          </div>
